...
def get_dataset_info(granularity_level, metric_type, all_results, dataset, methods, **kwargs):
    base_metrics = list(all_results[dataset][list(all_results[dataset].keys())[0]][f'{granularity_level}_metrics'].keys())
    ...
    if granularity_level == "single_table":
        if metric_type == "distance":
            ...
            raise ValueError(f"Unknown metric type {metric_type}. Should be either 'distance' or 'detection'.")
        ...
        raise ValueError(f"Unknown granularity level {granularity_level}. Should be either 'single_table' or 'single_column'.")
