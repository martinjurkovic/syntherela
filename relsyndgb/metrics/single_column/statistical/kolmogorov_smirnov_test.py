...
class KolmogorovSmirnovTest(StatisticalBaseMetric, SingleColumnMetric):
    ...
    def validate(self, column):
        ...
        if np.issubdtype(column_dtype, np.number) or np.issubdtype(column_dtype, np.datetime64):
            return

        raise ValueError(f"{self.name} can only be applied to numerical columns, but column {column.name} is of type {column.dtype}")
