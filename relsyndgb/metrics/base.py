...
class MachineLearningEfficacyBaseMetric(BaseMetric):
    ...
    def prepare_data(self, X, ht = None, **kwargs):
        if ht is None:
            ht = CustomHyperTransformer()
            ht.fit(X)
        transformed_data = ht.transform(X)
        X = transformed_data
        # replace infinite values with NaN
        X.replace([np.inf, -np.inf], np.nan, inplace=True)
        # drop constant columns
    ...
    def run(self, real_data, synthetic_data, metadata, test_data, **kwargs):
        ...
        scores_real = self.compute(X_real, y_real, X_test, y_test, random_state=self.random_state)
        scores_synthetic = self.compute(X_synthetic, y_synthetic, X_test, y_test, random_state=self.random_state + 1 if self.random_state else None)
