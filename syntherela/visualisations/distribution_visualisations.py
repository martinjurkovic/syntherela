...
from syntherela.visualisations.utils import get_bins
...
def visualize_marginals(real_data, synthetic_data, metadata):
    ...
    for table in metadata.get_tables():
        ...
        if num_non_id_columns >= 3:
            num_non_id_columns = (num_non_id_columns +
                                  (3 - num_non_id_columns % 3)
                                  if num_non_id_columns %
        ...
        for column, column_info in table_meta["columns"].items():
            ...
            data = pd.DataFrame(
                pd.concat(
                    ...
                    axis=0))
            data["Kind"] = ["Real"] * len(
                real_data[table]) + ["Synthetic"] * len(synthetic_data[table])
            ...
            elif (column_info["sdtype"] == "numerical"
...
def visualize_bivariate_distributions(real_data, synthetic_data, metadata):
    ...
    for table in metadata.get_tables():
        ...
        pairs = [(non_id_columns[i], non_id_columns[j])
                 for i in range(len(non_id_columns))
                 for j in range(i + 1, len(non_id_columns))]
        if len(pairs) == 0:
            continue
        ...
        for i, pair in enumerate(pairs):
            ...
            data_synthetic = pd.DataFrame({
                pair[0]:
                synthetic_data[table][pair[0]],
                pair[1]:
                synthetic_data[table][pair[1]],
            })
            ...
            sns.histplot(data=data_real,
                         ...
                         bins=(binsx, binsy))
            ax1.set_title("Real")
            sns.histplot(data=data_synthetic,
                         x=pair[0],
                         y=pair[1],
                         ax=ax2,
                         bins=(binsx, binsy))
            ax2.set_title("Synthetic")
            if (type(binsx) is int and binsx > 16
...
def visualize_parent_child_bivariates(real_data, synthetic_data, metadata):
    ...
    for table in metadata.get_tables():
        ...
        for i, pair in enumerate(pairs):
            ...
            data_real = pd.DataFrame({
                pair[0]:
                real_data[parent_table][parent_column],
                pair[1]:
                real_data[table][pair[1]],
            })
            data_synthetic = pd.DataFrame({
                pair[0]:
                synthetic_data[parent_table][parent_column],
                pair[1]:
                synthetic_data[table][pair[1]],
            })
            binsx = get_bins(data_real[pair[0]])
