...
class Report:
    ...
    def generate(self):
        ...
        """
        column_count = sum([
            len(self.real_data[table].columns)
        ...
        if len(self.single_column_metrics) == 0:
                ...
                for table in self.metadata.get_tables():
                    for metric in self.single_column_metrics:
                        for column, column_info in self.metadata.tables[
                            ...
                            try:
                                self.results[
                                    "single_column_metrics"].setdefault(
                                        metric.name, {}).setdefault(
                                            table, {})[column] = metric.run(
            ...
            if self.evaluate_trends:
                self.compute_trends(single_column=True,
                                    single_table=False,
                                    multi_table=False)
        # single_table_metrics
            ...
            with tqdm(
                ...
                for table in self.metadata.get_tables():
                    for metric in self.single_table_metrics:
                        ...
                        pbar.update(1)
            if self.evaluate_trends:
                self.compute_trends(single_column=False,
                                    single_table=True,
                                    multi_table=False)
        # multi_table_metrics
        if len(self.multi_table_metrics) == 0:
            ...
            for metric in tqdm(self.multi_table_metrics,
                               desc="Running Multi Table Metrics"):
                try:
    ...
    def visualize_distributions(self,
        ...
        """
        if marginals:
            visualize_marginals(self.real_data, self.synthetic_data,
                                self.metadata)
        if bivariate:
            visualize_bivariate_distributions(self.real_data,
                                              self.synthetic_data,
                                              self.metadata)
        if parent_child_bivariate:
            visualize_parent_child_bivariates(self.real_data,
                                              self.synthetic_data,
                                              self.metadata)
    ...
    def get_metric_instance(self, metric_name):
        ...
        """
        for metric in (self.single_column_metrics + self.single_table_metrics +
