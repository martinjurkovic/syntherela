...
for relationship in metadata.relationships:
    ...
    real_data[relationship['parent_table_name']].rename(columns={parent_key: join_on_name}, inplace=True)
    real_data[relationship['child_table_name']].rename(columns={child_key: join_on_name}, inplace=True)
...
join_on = metadata.get_primary_key(root_table_name) if len(renamed_keys.keys()) == 0 else join_on_name
parent_model_path = f'models/{MODEL_NAME}/{dataset_name}/checkpoint_{root_table_name}_run_{run_id}' 
...
for i in range(1,4):
    ...
    for child_table_name in sorted(metadata.get_children(root_table_name)):
        ...
        if retrain or not(os.path.exists(child_model_path) and len(os.listdir(child_model_path)) > 0):
            ...
            logger.debug(f"Skipping child table {child_table_name} generation and reading last trained model...")
        ...
        child_primary_key = metadata.get_primary_key(child_table_name) if child_table_name not in added_keys else added_keys[child_table_name]
        ...
        for table in synthetic_data:
            ...
            if table in renamed_keys:
                synthetic_data[table].rename(columns={join_on_name: renamed_keys[table]}, inplace=True)
