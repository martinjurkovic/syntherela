...
if retrain or not (os.path.exists(parent_model_path)
    ...
    parent_model = REaLTabFormer(
        ...
        checkpoints_dir=
        f"checkpoints/{MODEL_NAME}/{dataset_name}/checkpoint_{root_table_name}_run_{run_id}",
    ...
    for child_table_name in sorted(metadata.get_children(root_table_name)):
        ...
        if retrain or not (os.path.exists(child_model_path)
            ...
            child_model = REaLTabFormer(
                ...
                checkpoints_dir=
                f"checkpoints/{MODEL_NAME}/{dataset_name}/checkpoint_{child_table_name}_run_{run_id}",
                random_state=int(run_id),
                report_to=report_to,
            )
