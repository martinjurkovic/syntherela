...
for dataset in datasets:
    ...
    for gnn_arch in gnn_architectures:
        ...
        for method in methods:
            if (
                dataset in data and method in data[dataset]
                and gnn_arch in data[dataset][method]
            ):
...
for dataset in datasets:
    ...
    for gnn_arch in gnn_architectures:
        for method in method_order:
            ...
            if (
                dataset in results and gnn_arch in results[dataset]
                and method in results[dataset][gnn_arch]
            ):
...
num_columns = len(
    available_methods
) + 3  # Dataset + GNN Arch + Score Type + Methods
latex_table = (
    "\\begin{table}[ht]\n\\centering\n\\begin{tabular}{c" + "c" * num_columns +
    "}\n"
)
latex_table += "\\toprule\n"
latex_table += "Dataset & GNN Architecture & & " + " & ".join(
    available_methods
) + " \\\\\n"
latex_table += "\\midrule\n"
...
for dataset_idx, dataset in enumerate(datasets):
    ...
    for gnn_arch in gnn_architectures:
        if (
            dataset in results and gnn_arch in results[dataset] and any(
                method in results[dataset][gnn_arch]
                for method in method_order if method in methods
            )
        ):
    ...
    for gnn_idx, gnn_arch in enumerate(available_gnn_archs):
        ...
        for method in available_methods:
            ...
            if original_method not in ["ORIGINAL"]:
                if (
                    dataset in results and gnn_arch in results[dataset]
                    and original_method in results[dataset][gnn_arch]
                ):
            ...
            if (
                ...
                if original_method not in ["ORIGINAL"]:
                    ...
                    is_global_best = (
                        ...
                        and dataset_global_best[dataset]['gnn_arch'] == gnn_arch
                    )
