...
# Set to True to read from hyperparameter_tuning_100 directory
USE_HYPERPARAMETER_TUNING_RESULTS = True

# Configuration for table highlighting
# Change this to adjust the highlight color (e.g., "blue!15", "yellow!8", etc.)
HIGHLIGHT_COLOR = 'green!10'
...
if USE_HYPERPARAMETER_TUNING_RESULTS:
    ...
    hyperparameter_dir = os.path.join(
        PROJECT_PATH, 'results', 'hyperparameter_tuning_100'
    )
    hyperparameter_files = glob.glob(
        os.path.join(hyperparameter_dir, 'hyperparameter_results_*.json')
    )
    ...
    for file_path in hyperparameter_files:
        ...
        if arch not in data[dataset]['ORIGINAL']:
            data[dataset]['ORIGINAL'][arch] = {}

        # Override the ORIGINAL method results with hyperparameter tuning results
        data[dataset]['ORIGINAL'][arch]['1'] = best_results
        data[dataset]['ORIGINAL'][arch]['2'] = best_results
        data[dataset]['ORIGINAL'][arch]['3'] = best_results
...
architectures = set()
methods = set()
for _dataset, method_data in data.items():
    for method, gnn_data in method_data.items():
        methods.add(method)
        for arch in gnn_data.keys():
            architectures.add(arch)

# Sort GNN architectures for consistent ordering
architectures = sorted(list(architectures))

# Process results: dataset -> arch -> method -> (mean, se)
results = {}

for dataset in datasets:
    results[dataset] = {}
    for arch in architectures:
        results[dataset][arch] = {}
        for method in methods:
            if (
                dataset in data
                and method in data[dataset]
                and arch in data[dataset][method]
            ):
                runs = data[dataset][method][arch]
                metric_values = []
                ...
                if metric_values:
                    ...
                    results[dataset][arch][method] = (mean_value, se_value)
...
for dataset in datasets:
    ...
    for arch in architectures:
        for method in method_order:
            ...
            if (
                dataset in results
                and arch in results[dataset]
                and method in results[dataset][arch]
            ):
                mean, se = results[dataset][arch][method]
                all_scores_for_dataset.append((mean, se, method, arch))
    ...
    if all_scores_for_dataset:
        ...
        best_mean, best_se, best_method, best_arch = best_score
...
)  # Dataset + GNN Arch + Score Type + Methods
latex_table = (
    '\\begin{table}[ht]\n\\centering\n\\begin{tabular}{c'
    + 'c' * num_columns
    + '}\n'
...
for dataset_idx, dataset in enumerate(datasets):
    ...
    for arch in architectures:
        if (
            ...
            and arch in results[dataset]
            and any(
                method in results[dataset][arch]
                for method in method_order
    ...
    num_archs = len(available_archs)

    for gnn_idx, arch in enumerate(available_archs):
        ...
        for method in available_methods:
            ...
            if original_method not in ['ORIGINAL']:
                if (
                    dataset in results
                    and arch in results[dataset]
                    and original_method in results[dataset][arch]
                ):
                    mean, se = results[dataset][arch][original_method]
        ...
        if gnn_idx == 0:
            row.append(f'\\multirow{{{num_archs}}}{{*}}{{{dataset_name}}}')
        else:
            row.append('')

        # GNN Architecture column
        arch_display = arch_rename.get(arch, arch)
        row.append(arch_display)

        # Score type column (multirow for first entry)
        if gnn_idx == 0:
            row.append(f'\\multirow{{{num_archs}}}{{*}}{{{score_type}}}')
        else:
            row.append('')
        ...
        for method in available_methods:
            ...
            if (
                ...
                if original_method not in ['ORIGINAL']:
                    ...
                    is_global_best = (
                        dataset in dataset_global_best
                        and dataset_global_best[dataset]['method'] == method
                        and dataset_global_best[dataset]['arch'] == arch
                    )
                    ...
                    row.append(
                        f'{base_score_part} $({baseline_scores[dataset]})$'
                    )
                else:
