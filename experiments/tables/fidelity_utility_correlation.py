...
for dataset in datasets_methods.keys():
    ...
    common_methods_c2st_rdl = [
        m for m in base_methods if m in per_run_c2st_results.get(dataset, {})
    ...
    for run_idx in range(NUM_RUNS):
        ...
        if len(common_methods_c2st_rdl) < 2:
            ...
            valid_pairs = [
                (c, r) for c, r in zip(c2st_scores_run, rdl_scores_run)
        ...
        if len(common_methods_card_rdl) < 2:
            ...
            card_scores_run = [
                per_run_cardinality_results[dataset]
                [method].get(run_idx, np.nan)
                for method in common_methods_card_rdl
            ]

            valid_pairs = [
                (c, r) for c, r in zip(card_scores_run, rdl_scores_run)
                if not np.isnan(c) and not np.isnan(r)
            ]
            if len(valid_pairs) < 2:
                s_corr, s_p, k_corr, k_p = np.nan, np.nan, np.nan, np.nan
        ...
        if len(common_methods_onehop_rdl) < 2:
            ...
            valid_pairs = [
                (o, r) for o, r in zip(onehop_scores_run, rdl_scores_run)
    ...
    for prefix, values_list, p_values_list in [
        ...
        agg_results[f"{prefix}_stderr"] = sem(
            values_list, nan_policy='omit'
        ) if len(valid_corrs) > 0 else np.nan
