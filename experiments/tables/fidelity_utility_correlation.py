...
for dataset, methods in datasets_methods.items():
    ...
    for method in methods + ['ORIGINAL']:
        ...
        for run in range(1, NUM_RUNS + 1):
            try:
                score = rdl_utility_results[dataset][method][str(run)][
                    evaluation_type
                ]
        ...
        per_run_rdl_utility_results.setdefault(dataset, {}).setdefault(
            method, {}
        )
...
for dataset, methods in datasets_methods.items():
    for method in methods:
        ...
        per_run_cardinality_results.setdefault(dataset, {}).setdefault(
            method, {}
        )
...
if (
    'rossmann_subsampled' in per_run_rdl_utility_results
    and 'CLAVADDPM' in per_run_rdl_utility_results['rossmann_subsampled']
):
    print(
...
if (
    'rossmann_subsampled' in per_run_c2st_results
    and 'CLAVADDPM' in per_run_c2st_results['rossmann_subsampled']
):
    print(
...
for dataset in datasets_methods.keys():
    ...
    common_methods_c2st_rdl = [
        ...
        if m in per_run_c2st_results.get(dataset, {})
        and m in per_run_rdl_utility_results.get(dataset, {})
    ]
    common_methods_card_rdl = [
        m
        for m in base_methods
        if m in per_run_cardinality_results.get(dataset, {})
        and m in per_run_rdl_utility_results.get(dataset, {})
    ]
    common_methods_onehop_rdl = [
        m
        for m in base_methods
        if m in per_run_one_hop_results.get(dataset, {})
        and m in per_run_rdl_utility_results.get(dataset, {})
    ]
    ...
    for run_idx in range(NUM_RUNS):
        ...
        if len(common_methods_c2st_rdl) < 2:
            ...
            valid_pairs = [
                (c, r)
                for c, r in zip(c2st_scores_run, rdl_scores_run, strict=False)
            ...
            if len(valid_pairs) < 2:
                ...
                c2st_v, rdl_v = zip(*valid_pairs, strict=False)
                try:
        ...
        if len(common_methods_card_rdl) < 2:
            ...
            valid_pairs = [
                (c, r)
                for c, r in zip(card_scores_run, rdl_scores_run, strict=False)
                if not np.isnan(c) and not np.isnan(r)
            ]
            if len(valid_pairs) < 2:
                s_corr, s_p, k_corr, k_p = np.nan, np.nan, np.nan, np.nan
                ...
                card_v, rdl_v = zip(*valid_pairs, strict=False)
                try:
        ...
        if len(common_methods_onehop_rdl) < 2:
            ...
            valid_pairs = [
                (o, r)
                for o, r in zip(onehop_scores_run, rdl_scores_run, strict=False)
            ...
            if len(valid_pairs) < 2:
                ...
                onehop_v, rdl_v = zip(*valid_pairs, strict=False)
                try:
    ...
    agg_results = {
        'message': '; '.join(list(set(dataset_messages)))
        if dataset_messages
        else None
    }
    ...
    for prefix, values_list, p_values_list in [
        ...
        agg_results[f'{prefix}_stderr'] = (
            sem(values_list, nan_policy='omit')
...
for dataset_name in datasets_methods.keys():
    ...
    k_c2st_str = format_latex_value(
        k_c2st_mean, k_c2st_stderr, k_c2st_n, NUM_RUNS
    )
    k_card_str = format_latex_value(
        k_card_mean, k_card_stderr, k_card_n, NUM_RUNS
    )
    k_onehop_str = format_latex_value(
        k_onehop_mean, k_onehop_stderr, k_onehop_n, NUM_RUNS
    )
    ...
    if dataset_level_message:
        if (
            'C2ST-RDL: Not enough common methods' in dataset_level_message
            and k_c2st_n == 0
        ):
            pass
        elif (
            'Card-RDL: Not enough common methods' in dataset_level_message
            and k_card_n == 0
        ):
            pass
        elif (
            'OneHop-RDL: Not enough common methods' in dataset_level_message
            and k_onehop_n == 0
        ):
            pass
        else:
            if (
                k_c2st_n < NUM_RUNS
                or k_card_n < NUM_RUNS
                or k_onehop_n < NUM_RUNS
            ):
                ...
                simplified_msg = (
                    simplified_msg.replace(';;', ';').strip().strip(';')
                )
