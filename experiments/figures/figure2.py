...
for method in methods:
    ...
    all_tables[method_name] = load_tables(f'data/synthetic/rossmann_subsampled/{method}/1/sample1', metadata)
    all_tables[method_name], metadata = remove_sdv_columns(all_tables[method_name], metadata, update_metadata=False)
...
data[column1] = data[column1].astype('object').map({
...
column_order = tables[table][column1].astype('object').fillna('missing').map({
    'Jan,Apr,Jul,Oct': 'J-A-J-O',
    'Feb,May,Aug,Nov': 'F-M-A-N',
    'Mar,Jun,Sept,Dec': 'M-J-S-D',
    'missing': 'missing'
...
sns.histplot(data=data.dropna(), x=column1, hue='Data', multiple='dodge', stat='density', common_norm=False, legend=True, ax=axes[0], shrink=0.8, palette=COLOR_DICT, alpha=0.8)

table = 'historical'
column2 = 'Customers'
# column2 = 'CompetitionDistance'
columns = [
...
sns.kdeplot(data=data, x=column2, hue='Data', common_norm=False, fill=False, legend=True, ax=axes[1], clip=(0, data[column2].max()), palette=COLOR_DICT, alpha=0.8)
...
plt.savefig('results/figures/figure2.png', dpi = 600)