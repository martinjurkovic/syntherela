...
def multi_comparison_chart(
    ...
    if runs is None:
        runs = list(single_table_results.keys())
    if names is None:
    ...
    deltas = [
        ...
        for multi_mean, single_mean in zip(
            multi_means, single_means, strict=False
        )
    ]
    ...
    for i in range(len(models)):
        if deltas[i] == min(deltas) and bold_best:
            ...
            ax.text(
                x[i] + d,
                (single_means[i] + multi_means[i]) / 2,
                f' Δ = {delta}',
                va='center',
                color='gray',
                fontweight='bold',
    ...
    ax.plot(
        [0, 0], [0.5, 0.5], '--', color='gray', zorder=0, label='Relational Δ'
    )
    if tabular_sota is not None:
        mean, std, method = tabular_sota
        ax.axhline(
            y=mean, color='C4', linestyle='--', alpha=0.8
        )  # Tabular SOTA line
        ax.fill_between(
            [-d, len(models) * width + d],
            mean - std,
            mean + std,
            color='C4',
            alpha=0.1,
    ...
    ax.axhline(
        y=0.5, color='C3', linestyle='--', alpha=0.8
