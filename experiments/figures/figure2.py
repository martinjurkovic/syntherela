...
def multi_comparison_chart(
    ...
    for i in range(len(models)):
        if deltas[i] == min(deltas) and bold_best:
            ...
            ax.text(
                x[i] + d, (single_means[i] + multi_means[i]) / 2,
                f' Δ = {delta}',
                va='center',
                color='gray',
                fontweight='bold'
        ...
        ax.plot(
            [x[i] + d, x[i] + d], [single_means[i], multi_means[i]],
            '--',
            color='gray',
            zorder=3
        )
        ax.plot(
            [x[i], x[i] + d], [single_means[i], single_means[i]],
            '--',
            color='gray',
            zorder=3
        )
        ax.plot(
            [x[i], x[i] + d], [multi_means[i], multi_means[i]],
            '--',
            color='gray',
            zorder=3
        )
