...
for i, method in enumerate(methods):
    dcrs_real = torch.load(f"results/dcr/{table}_{method}_dcrs_real.pt",
                           weights_only=True).numpy()
    dcrs_test = torch.load(f"results/dcr/{table}_{method}_dcrs_test.pt",
                           weights_only=True).numpy()
    bins = np.histogram_bin_edges(np.concatenate((dcrs_real, dcrs_test)),
                                  bins=bins)
    ...
    plt.savefig(f"results/figures/dcr/figure5{chr(ord('`')+(i+1))}.png",
                dpi=300,
                bbox_inches='tight')
    plt.clf()
