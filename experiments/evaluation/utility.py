...
def process_airbnb(tables, metadata, categories):
    ...
    sessions_data = (
        tables['sessions'][['secs_elapsed', 'user_id']]
        .groupby('user_id')
        .mean()
    )
...
if __name__ == '__main__':
    ...
    for method in methods_to_run:
        ...
        for classifier, classifier_cls in classifiers[task].items():
            ...
            feature_importance_real = real_scores[classifier][
                'feature_importance'
            ]
        ...
        results[dataset_name][method]['feature_importance_spearman_se'] = (
            np.std(feature_importances_spearman) / np.sqrt(m)
        )
        ...
        results[dataset_name][method]['feature_importance_weighted_se'] = (
            np.std(feature_importances_weighted) / np.sqrt(m)
        )
