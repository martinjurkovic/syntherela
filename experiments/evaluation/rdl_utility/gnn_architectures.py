...
class HeteroGNN(torch.nn.Module):
    def __init__(
        ...
        **conv_kwargs,
        ...
        """Heterogeneous GNN that uses different convolution layers.
...
def gin_conv_factory(
    in_channels: int, out_channels: int, aggr: str = 'mean', **kwargs
):
    """Create GIN convolution layers."""
...
def graphconv_factory(in_channels: int, out_channels: int, **kwargs):
    """Create GraphConv layers (GCN-like but supports heterogeneous graphs)."""
...
def gat_conv_factory(
    ...
    **kwargs,
    ...
    return GATConv(
        in_channels, head_out_channels, heads=heads, concat=concat, **kwargs
    )


def gatv2_conv_factory(
    in_channels: int,
    out_channels: int,
    heads: int = 4,
    concat: bool = True,
    **kwargs,
    ...
    """Create GAT v2 convolution layers."""
    # Set add_self_loops=False for heterogeneous graphs to avoid HeteroConv issues
    kwargs.setdefault('add_self_loops', False)
    if concat:
        # When concatenating heads, each head should output out_channels/heads features
