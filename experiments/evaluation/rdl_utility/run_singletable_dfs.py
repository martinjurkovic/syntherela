...
for split, table in [
    ...
    print('Adding tables to EntitySet...')
    ...
    for table_name, table_obj in db.table_dict.items():
        if (
            hasattr(table_obj, 'fkey_col_to_pkey_table')
            and table_obj.fkey_col_to_pkey_table
        ):
            for (
                fkey_col,
                parent_table,
            ) in table_obj.fkey_col_to_pkey_table.items():
                if (
                    parent_table in es.dataframe_dict
                    and table_name in es.dataframe_dict
                ):
                    try:
                        parent_obj = db.table_dict[parent_table]
                        parent_pkey = (
                            parent_obj.pkey_col
                            if parent_obj.pkey_col
                        ...
                        print(
                            f'  ✓ {parent_table}.{parent_pkey} -> {table_name}.{fkey_col}'  # noqa: E501
                        )
                        relationships_added += 1
                    except Exception as e:
                        print(
                            f'  ✗ Failed relationship {parent_table} -> {table_name}: {e}'  # noqa: E501
                        )
