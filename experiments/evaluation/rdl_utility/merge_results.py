"""Merge RDL Utility results.

This script merges individual dataset result files into a single combined
results file. Run this after the tmux benchmark completes.
"""
            ...
            for method in merged_results[dataset]:
                for gnn_arch in merged_results[dataset][method]:
                    for run_id in merged_results[dataset][method][gnn_arch]:
                        if (
                            merged_results[dataset][method][gnn_arch][run_id]
                            != {}
                        ):
                            total_experiments += 1
